...
class Database {
    ...
    async seedDefaultOwners() {
        // Check if default owners have already been seeded (even if user deleted them later)
        const hasSeededOwners = await this.getSetting('defaultOwnersSeeded');
        if (hasSeededOwners) {
            console.log('Default owners already seeded before, skipping');
            return;
        }
        ...
        if (existingOwners.length === 0) {
            ...
            // Mark that default owners have been seeded
            await this.saveSetting('defaultOwnersSeeded', true);
        }
    }
    ...
    async seedSampleProducts() {
        // Check if sample products have already been seeded (even if user deleted them later)
        const hasSeededProducts = await this.getSetting('sampleProductsSeeded');
        if (hasSeededProducts) {
            console.log('Sample products already seeded before, skipping');
            return;
        }
        ...
        if (existingProducts.length === 0) {
            ...
            // Mark that sample products have been seeded
            await this.saveSetting('sampleProductsSeeded', true);
        }
    }
}
